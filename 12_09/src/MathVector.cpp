...
MathVector::MathVector(int dim,void *arr) : MathVector()
{
    init(dim,(double*)arr);
}
...
MathVector& MathVector::operator=(const MathVector &&input){
    ...
    if(this!=&input){
        ...
        input._arr=nullptr; //avoid free() Segmentation
    }
    return *this;
}
// v2+v3 -> input = v3 , function is v2
MathVector MathVector::operator+(const MathVector &input) const
{
    if(this->_arr==nullptr || this->_dim==0){
        return MathVactor(input);
        //return input is error 記憶體空間有問題(return  and original arr 會共用記憶體)
    }
