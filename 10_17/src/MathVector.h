...
public:
    ...
    // call MathVector() function(default constructor) first , then go inside the function
    MathVector(int dim,double *arr) : MathVector() // inline function call
    {
        std::cout << "constructor" <<std::endl;
        init(dim,arr);
    }
    MathVector(const MathVector &input) : MathVector(input._dim,input._arr)
    {   // copy constructor,pass by reference('&'):avoid copy
